...
import GradientText from "@/components/desktop/misc/GradientText";
import { DashboardSkeleton } from "@/components/desktop/skeletons/DashboardSkeleton";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const DashboardHome = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [wakaResult, , weeklyResult, monthlyResult] =
        await Promise.allSettled([
          fetch("/api/wakatime?range=last_30_days").then((r) =>
            r.ok ? r.json() : null,
          ),
          ...
          (async () => {
            ...
            const res = await fetch(
              `/api/wakatime/summaries?start=${startStr}&end=${endStr}`,
            );
            if (!res.ok) return [];
          ...
          (async () => {
            ...
            const res = await fetch(
              `/api/wakatime/summaries?start=${startMonthStr}&end=${endStr}`,
            );
            if (!res.ok) return [];
            const json = await res.json();
            return json.data || [];
      ...
      if (weeklyResult.status === "fulfilled")
        setWeeklySummaries(weeklyResult.value);
      if (monthlyResult.status === "fulfilled")
        setDailyBreakdown(monthlyResult.value);
      setMainReady(true);
    };
  ...
  const dailyAverage = parseTimeToHours(
    wakaData?.human_readable_daily_average ?? "0",
  );
  const targetDaily = 3;
            ...
            <Card className="bg-zinc-900 text-white border-gray-700 cursor-pointer hover:border-blue-500/50 transition-colors">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Clock className="w-5 h-5 text-yellow-400" /> Total Coding
                  Hours
                </CardTitle>
              </CardHeader>
              <CardContent>
