...
import { CodingSkeleton } from "@/components/desktop/skeletons/CodingSkeleton";
import { Calendar } from "@/components/desktop/ui/calendar";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from "@/components/desktop/ui/popover";
...
export default function CodingAnalyticsPage() {
    ...
    try {
      if (range === "custom" && customDate) {
        ...
        const res = await fetch(
          `/api/wakatime/summaries?start=${dateStr}&end=${dateStr}`,
        );
  ...
  const projects = (wakaData?.projects ?? []).sort(
    (a: Project, b: Project) => b.total_seconds - a.total_seconds,
  );

  const totalLangSeconds = topLangs.reduce(
    (sum, l) => sum + l.total_seconds,
    0,
  );
                ...
                plugins: {
                  ...
                  tooltip: {
                    callbacks: {
                      label: (context) => {
                        const seconds =
                          topLangs[context.dataIndex]?.total_seconds ?? 0;
                        const pct =
                          totalLangSeconds > 0
