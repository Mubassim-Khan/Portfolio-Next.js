...
const SEARCH_QUERIES: Record<FilterType, string> = {
  merged: "author:Mubassim-Khan type:pr is:merged",
  open: "author:Mubassim-Khan type:pr is:open",
  closed: "author:Mubassim-Khan type:pr is:closed is:unmerged",
};
    ...
    try {
      ...
      if (data.data?.search?.edges) {
        const fetchedPRs = data.data.search.edges
          ...
          .filter((pr: PR) => !(pr.title === "Main" && pr.repository.nameWithOwner === "Mubassim-Khan/flexprice-storybook"));
