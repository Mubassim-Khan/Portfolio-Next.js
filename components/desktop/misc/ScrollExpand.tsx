...
export interface ScrollExpandProps {
  /** Image or video source URL. If omitted, children become the primary frame content. */
  src?: string;
  ...
  title?: string;
  scrollHint?: string;
  /** Starting frame width as % of viewport (0–100). */
  startWidth?: number;
  /** Starting frame height as % of viewport (0–100). */
  startHeight?: number;
  startRadius?: number;
  endRadius?: number;
  mediaZoom?: number;
  /** How many viewport-heights of scroll the expand takes. */
  scrollDistance?: number;
  /** Extra hold distance (viewport-heights) after expansion completes. */
  holdDistance?: number;
  smoothing?: number;
  overlayScrim?: number;
...
const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  const applyProgress = useCallback(
    (p: number) => {
      ...
      // Media zoom (only when we have a media element)
      ...
      // Overlay children (only in media mode — fades in late)
  ...
  // ---------- Frame content differs based on media vs. children-only mode ----------

  const renderFrameContent = () => {
    if (hasMedia) {
      // Classic mode: media + scrim + children as late-appearing overlay
      return (
        <>
          {renderMedia()}
          <div
    ...
    // Children-only mode: children ARE the primary content inside the clipped frame.
    // They are always visible (no opacity animation) — the clip-path reveals them.
    return (
      ...
      <div ref={trackRef} className="relative w-full">
        ...
        >
          ...
          <div
            ...
            style={{
              clipPath: `inset(${(100 - startHeight) / 2}% ${(100 - startWidth) / 2}% ${(100 - startHeight) / 2}% ${(100 - startWidth) / 2}% round ${startRadius}px)`,
          ...
          {/* Title overlay (above the frame, fades out on scroll) */}
          {title ? (
            <div
              ref={titleRef}
